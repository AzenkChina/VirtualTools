...
int CGXDLMSClient::Method(CGXDLMSObject* item, int index,
    CGXByteBuffer& data, std::vector<CGXByteBuffer>& reply)
{
    int ret;
    if (index < 1)
    {
    ...
    {
        // CI
        bb.SetUInt16(item->GetObjectType());
        // Add LN.
        unsigned char ln[6];
        GXHelpers::SetLogicalName(item->GetName().strVal.c_str(), ln);
        bb.Set(ln, 6);
        // Attribute ID.
        bb.SetUInt8(index);
    ...
    else
    {
        ...
        if ((ret = CGXDLMS::GetActionInfo(item->GetObjectType(), ind, count)) != 0)
        {
            return ret;
        }
